    ...
    public void SearchForFio(String fio) {
        ...
        for (int i = 0; i < students.length; i++) {
            if (students[i].getFio()==fio) {
                System.out.println(students[i].getId()+" "+students[i].getAverage()+" "+students[i].getFio()); check = 1;
            }
        }
        if (check==0) {
        ...
        for (int i = 0; i < IDNumber.length - 1; i++) {
            for (int j = IDNumber.length - 1; j > i; j--) {
                if (IDNumber[j - 1] > IDNumber[j]) {
                    double tmp = IDNumber[j - 1]; IDNumber[j - 1] = IDNumber[j]; IDNumber[j] = (int) tmp;
                }
            }
        }
        for (int i = 0; i < 5; i++) {
